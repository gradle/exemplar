...
or maybe a more complex, multistep sample:
...
*NOTE:* `GradleSamplesRunner` supports Java 17 and above.
...
You might want to verify more than just log output, so this library includes link:https://github.com/junit-team/junit4/wiki/rules[JUnit rules] that allow you to easily copy sample projects to a temporary location for other verification. Here is an example of a test that demonstrates use of the `@Sample` and `@UsesSample` rules.
...
- Added `GradleTaskFailureBuildFileNormalizer` to normalize task provenance information in task failure messages.

=== 2.0.0

- Updated AsciidoctorJ dependency to version 3.0.1
- Updated minimal supported Java version to 17
